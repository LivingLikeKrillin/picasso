...
class InputCollectorTest {
    ...
    @Test
    fun `검사 아홉이 전부 목록에 있고 id가 겹치지 않는다`() {
        // §11.2는 열이다. 10번은 검사가 아니라 음성 하네스라 목록에 없다 —
        // 그 사실이 여기 적혀 있어야 "아홉이면 다 됐다"로 읽히지 않는다.
        //
        // ★**정렬이 문자열이다.** 검사가 열을 넘는 날 "10" 이 "2" 앞에 오므로, 그때 이 줄이
        //   먼저 빨개지고 id 를 어떻게 셀지 다시 정하게 된다.
        val ids = GateChecks.all().map { it.id }
        assertEquals(listOf("1", "2", "3", "4", "5", "6", "7", "8", "9"), ids.sorted())
    }
