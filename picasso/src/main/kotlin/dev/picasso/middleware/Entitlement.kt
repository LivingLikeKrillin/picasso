 ...
 * 선언이 **철회됐다는 기록**(ADR 45).
 *
 * ## 철회는 지우는 것이 아니라 표시하는 것이다
 *
 * 지우면 [Entitlements.declaredFor] 가 널을 돌려주고, 그것은 «아직 안 올렸다» 와 같은 모양이다.
 * 운영자가 받는 다음 행동이 정반대인데도 그렇다 — 철회는 **무언가 바뀌었다**는 뜻이라 사후 검토로 가야
 * 하고, 미선언은 **원래 없었다**는 뜻이라 선언을 올리러 가야 한다. 접으면 강등된 사람을 선언 수정하러
 * 보낸다.
 *
 * ## 이 층은 이것을 쓰지 않는다
 *
 * 자동 승인한 조치가 실패한 사실은 사건 번들의 `approvedBy` 로 이미 밖에 나간다(§15.168). 그것을 보고
 * 철회할지 정하는 것은 **선언의 주인**이고, 이 층은 그 결과를 읽어 거절의 종류로 옮길 뿐이다. 이 층이
 * 쓰면 소유하지 않은 값을 고치는 것이고, 억제 목록을 따로 두면 자격의 원천이 둘이 된다(ADR 45).
 */
data class Revocation(
    val at: Instant,
    /** 누가 철회했는가. [Approver] 와 같은 성질이다 — 위조 가능하며 조사 단서다(§15.3). */
    val by: String,
    /**
     * 왜 철회했는가. **사람이 읽을 산문이다** — 읽는 쪽이 갈래를 가르는 데 쓰는 것은 거절의 종류이지
     * 이 문장이 아니다. 문자열로 대조하면 문구를 고치는 날 조용히 안 걸린다.
     */
    val reason: String,
)
...
data class Entitlement(
    ...
    /**
     * 철회됐으면 그 기록, 아니면 널(ADR 45). **밖이 쓰고 이 층은 읽는다.**
     *
     * 철회된 선언도 [Entitlements.declaredFor] 가 그대로 돌려준다 — 그것이 이 칸의 요점이다. 빼 버리면
     * 「철회됐다」가 「원래 없었다」와 같은 답이 되고, 그 둘은 다음 행동이 반대다.
     */
    val revocation: Revocation? = null,
) {
