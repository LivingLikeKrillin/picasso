...
`Seeded` 가 `private` 인 것이 그 규율의 집행이다. 공개하면 전송·제어 코드가 그대로 잡을 수 있고, 실측으로
...
| `profile/` · `RegistrySource.kt` | 프로파일을 파일에서 읽거나 레지스트리에서 **당긴다**(5 초) |
...
대가가 둘 적혀 있다: 제어 채널의 proto 는 `buf lint`·`breaking` 밖이고(§15.32 — 그래서 RPC 이름과 번호를 미리
