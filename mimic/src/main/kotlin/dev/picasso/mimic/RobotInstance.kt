...
class RobotInstance(
     ...
     *
     * **프로파일에서 오지 않는다.** 프로파일은 기종을 기술하고 이름은 사이트가
     * 저작한 것이라, 여기 넣으면 같은 기종이 사이트마다 다른 문서를 갖게 된다.
     * 제어 채널이 심는다(§10.5) — 런타임 사실이기 때문이다.
     *
     * `null`은 **이름을 호스팅하지 못하는 기종**이다. 빈 목록(하나도 등록
     * 안 됨)과 반드시 구별한다 — 접으면 등록할 자리가 없는 기체에게 등록을
