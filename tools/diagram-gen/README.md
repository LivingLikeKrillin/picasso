...
node tools/diagram-gen/<이름>.mjs <블로그경로>/<이름>.svg 1180    # 블로그 (.dg 가 프로즈 열을 벗어난다)
```

**왜 생성기인가.** 같은 그림이 두 폭으로 필요하고, 손으로 두 벌 두면 어느 날 한쪽만 고쳐진다.
1180 을 저장소에 그대로 넣으면 글자가 바닥(9.5px) 밑으로 내려가므로 복사로는 안 된다.
