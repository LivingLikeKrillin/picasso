...
```bash
# Spot — protos/bosdyn/api 아래에서 우리가 쓰는 12 개를 한 디렉터리로 모은 뒤
...
# G1 — `g1_loco_api.hpp` 와 IDL 생성 파이썬을 한 디렉터리에
...
| | 원본 | 덮는 것 | 못 덮는 것 |
|---|---|---|---|
| Spot | `.proto` 12 개 | 메시지·필드·열거값·RPC | — |
| Digit | SDK `json.py` + 매뉴얼 본문 | 보내는 것(SDK) + 받는 것(매뉴얼) | 일반 오류 봉투의 모양(어디에도 없다) |
| G1 | C++ API ID 상수 + `Jsonize*` 의 JSON 키 + IDL | 무엇이 있는지, 요청 본문의 키 | **응답의 모양** — 어디에도 없다. 키에 타입·필수 여부가 안 딸려 온다 |
