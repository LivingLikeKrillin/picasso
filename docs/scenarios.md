...
## 2b. 임무 하나가 원자 단위 열이 되는 자리

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="diagrams/mission.dark.svg">
  <img alt="위에 상류가 보내는 JobOrder 한 칸이 있고, 거기서 화살표 셋이 능력 셋으로 갈라진다. 능력마다 주문에서 무엇을 읽는지와 어떻게 나누는지가 적혀 있고, 그 아래에 실제로 내는 실행 단위가 칸으로 늘어선다 — 부품 시퀀싱은 슬롯 수만큼, 용기 공급은 하나, 설비 점검은 대상마다 둘. 맨 아래 줄에 능력마다의 최고 근거 등급, 선택 파라미터, 쪼갤 수 없을 때의 실패 이름이 있다." src="diagrams/mission.svg">
</picture>

**상류는 결과만 말한다.** 어느 로봇을 쓸지, 어느 스킬로 할지, 몇 개로 쪼갤지는 주문에 없다 — 있는 것은
`workMasterId` 와 `EquipmentRequirement` 목록, 그리고 요구하는 근거 등급이다. 쪼개는 규칙은 `LogicalCapability`
구현 하나 안에 있고, **공통 엔진은 능력을 더해도 안 바뀐다**(17장 10번).

**조건도 그 한 장이 함께 든다.** 최고 근거 등급 · 시간창 δ · `IN_DOUBT` 유예 · 정체 창 · 쓰고 싶은 선택
파라미터가 전부 능력의 것이고 현장마다 다르게 준다. **요구 등급이 그 능력의 최고 등급을 넘으면 주문을 아예
접수하지 않는다** — 확인할 수단이 없으면 제공할 수 없다는 규칙(보고서 11.3)이 여기서 집행된다.
...
> 마지막 대조: 2026-09-11 · sha256:b5cedbede863 · 열림: ADR 32 · 시나리오 5, 시나리오 §8, §15.126, §15.87, §15.81, §15.8, C-3
