...
|---|---|---|---|---|
| ① | 부품 용기를 창고 출고 버퍼에서 시퀀싱 셀 입고 위치로 공급 | WMS | **AMR 플릿** | **아니다.** 결과가 ② 의 환경 전제가 된다 |
...
| 배정 · 조합 · 완료 판정 | ② 어휘 · ③ 배정과 결정 | **경계 안의 층, 인스턴스는 배치**(§9.6) | 일감을 실행 단위로 나누고, 어느 기체에 줄지 정하고, 완료를 판정한다 |
...
1. **picasso 는 AMR 을 어댑터로 감싸지 않는다.** 그 자리에는 이미 표준과 관제가 있다. ADR 36 의 등재 기준(발명 금지)은 반대 방향으로도 읽힌다 — **이미 있는 것을 다시 만들지 않는다.** 이 저장소의 어댑터는 휴머노이드와 4족이고, 그 둘은 그런 표준이 없어서 이 저장소가 있는 것이다.
...
| 작업 중 생산 순서가 버전 18 로 변경 | 기존 실행을 몰래 덮어쓰지 않음. 중단 가능 지점과 현재 랙 상태를 보고 | 종착한 슬롯의 태스크는 갱신 거절(`INVALID_TRANSITION`) — *"완료 슬롯은 새 버전이 상속"* 이 저절로 성립. 미시작 슬롯은 새 `revision` 으로 갱신 | **도는 슬롯(S03)** — §4.4 갱신 규칙이 `RUNNING` 에서 `Halt → Reset → Start` 인데, 부품을 든 채의 `Reset` 이 무엇인지 계약이 말하지 않는다. **§7 후보 ③** |
| 부품을 든 채 연결이 끊김 | 결과 미확정으로 관리하고 로봇 상태를 재조회 | 연결 스트림이 `OFFLINE`/`CONNECTION_BROKEN` 을 가른다(완료 기준 5). **같은 `(task_id, revision)` 재전송은 같은 핸들**이므로 *"접수됐는가"* 는 계약이 답한다 — 벤더가 클라이언트 참조 키를 안 받아도 어댑터가 그 매핑을 든다 | **무엇을 들고 있는가.** `partial_result` 자유 문자열뿐이다. **§7 후보 ②.** 그리고 어댑터가 재시작하면 그 매핑이 사라진다(§1.3 B-1 비목표와 맞닿는다) |

### 4.5 버전과 지연 이벤트

외부 문서의 규칙을 받는다 — 결과 이벤트는 `(실행 ID, 작업 버전)` 으로 맞추고, 어긋나는 이벤트는 폐기가 아니라 **보존**한다. 버전 17 을 돌리다 18 로 바꿨는데 뒤늦게 도착한 17 의 완료로 18 을 닫으면 안 된다.

picasso 의 `(task_id, revision)` 이 그 짝이다. 요청 쪽 규칙은 §4.4 에 있다(낮으면 `OUTDATED_REVISION`). **이벤트 쪽 규칙 — 옛 revision 을 단 `TaskUpdate` 가 새 revision 뒤에 도착했을 때 — 는 적혀 있지 않다.** 이벤트가 `revision` 을 싣고 있으므로 소비자가 구분할 수는 있다. **§7 후보 ④.**
...
| 4 | 부분 완료 후 취소 | **반** | `CANCELLING → CANCELLED / CANCELLED_RECOVERY_FAILED`(완료 기준 8b) 는 있다. **중단점·잔여 파지 상태는 없다** |
...
열 중 여섯을 `mimic` 위에서 이미 증명하고, 셋은 의도적으로 계약 밖이며, **하나(4)만 계약이 반쯤 답한다.** 그 반이 §7 의 후보 ② 다.
...
| ② | **취소·실패 시 잔여 물리 상태** — 무엇을 들고 있는가, 어느 단위까지 끝났는가 | `partial_result` 자유 문자열 | 벤더가 준다 — Spot `is_gripper_holding_item`·`CarryState`, G1 `press_sensor_state`, Digit `get-execution-state`. 발신자가 있으므로 ADR 9 를 통과한다 |
...
**이 표를 계약에 넣지 않은 이유.** ADR 9 — 소비 표면이 없는 선언은 두지 않는다. ①은 층 ③ 이 서야 소비자가 생기고, ②는 발신자가 있으니 가장 먼저 열 수 있다. 순서는 ② → ① 이며, 둘 다 결과 어휘 정준화의 일부로 다룬다.
