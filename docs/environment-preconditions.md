...
**등급.** 아래 근거는 전부 벤더 1차 자료(Spot 공개 SDK proto 152개 8089심볼, Digit 매뉴얼 409심볼)에서 왔다. 심볼 이름은 `adapter-*/src/test/resources/vendor-manifest.txt`와 대조된다.
...
**기종이 다르면 전제도 다르다.** 아래는 Digit 매뉴얼·SDK 409심볼 실측이며, 앞의 A~J와 짝을 맞춰 읽는다.

## 대비표 — 이것이 이 문서의 요점이다

| | Spot (8089심볼) | Digit (409심볼) | G1 (161심볼) |
|---|---|---|---|
...
> **앞 판이 44심볼로 재고 *"전제가 셋"*이라 적었다. 틀렸다.** 그것은 G1 서비스 넷 중 `loco` 하나만 본 것이었다. 넷 전부와 `terminations.hpp`, `unitree_hg` IDL 열하나를 넣어 **141심볼**로 다시 쟀다. **Spot을 54개 중 3개만 보고 쟀던 것(§15.75)과 같은 실수였고, 하루 만에 두 번째다.** 그 뒤 추출기에 **C++ 전용 IDL 용 리더**를 더해 `hg/SportModeState_`·`hg/AgvBmsState_`·`hg_doubleimu/doubleIMUState_`를 마저 읽었고, 최종값은 **161심볼**이다(`profile/distance/unitree-g1.json`).
>
...
지도·이름·객체·도크·인지가 여전히 **이 표면에 없다.** 그러므로 앞 절의 결론은 살아남는다 — **G1은 환경을 아는 일감을 못 받는다.** 다만 그 이유가 *"기체가 단순해서"*가 아니라 **"그 표면이 저작된 환경을 안 다뤄서"**이고, `go2` 네임스페이스(`HeightMap_`·`VoxelMapCompressed_`·`LidarState_`·`UwbState_`)와 ROS2 nav 메시지(`OccupancyGrid_`·`Odometry_`)는 **이미 확인했다 — 이 기종 것이 아니다(2026-09-09 확인, `profile/distance/unitree-g1.json`).** 다른 네임스페이스(`unitree_go`)에 있고 `hg`에 대응물이 없으며, G1 예제 열여덟 어디에도 안 쓰인다(Go2 예제조차 안 쓴다).
...
- ~~**G1의 범위가 너무 좁다.** 44심볼로 잰 것이라 *"전제가 셋"*이 실제인지 우리가 좁게 본 것인지 구별되지 않는다.~~ **닫혔다** — 서비스 넷 전부와 IDL을 다 넣어 161심볼로 재측정했다(위 "G1 — 범위를 넓히니 전제가 셋에서 여덟로 늘었다" 절).
- 안전 격리(사람과 로봇의 공간 분리)는 여기 안 적었다 — [ADR 32](adr/0032-safety-boundary.md)가 안전을 계약 밖으로 둔 것과의 관계를 먼저 정해야 한다.

> 마지막 대조: 2026-09-11 · sha256:73af884ba53d · 열림: §15.81, §15.79, §15.2, C-3
