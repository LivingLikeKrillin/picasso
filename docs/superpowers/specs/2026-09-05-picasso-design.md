...
142. **전체를 한 눈에 보는 자료가 없었다 — 그림 열한 장이 전부 단면이었다.**
    ...
    **자리는 둘이다.** README 의 「무엇이 있나」 절 머리(디렉터리 나무 위)와 `architecture.md` §4b(표 위). 같은 사실을 그림과 표로 두 번 보여 주는 것이고, 둘 다 같은 빌드 파일에서 나온다.
    ...
    ★**라우팅이 두 판 걸렸다.** 한 줄을 건너뛰는 간선 넷이 가운데 줄의 칸들을 관통했다(검증기가 여덟 건 잡았다). 줄을 좁혀 양옆에 채널을 내고 그 넷만 가장자리로 돌렸다. 그래도 채널이 가운데 줄을 감싸는 틀처럼 보여서, 맨 윗줄을 다른 줄과 같은 폭으로 맞춰 채널까지의 거리를 줄였다.
