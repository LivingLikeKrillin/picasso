...
140. **블로그에 있던 그림 아홉 중 셋을 저장소로 옮겼다 — 옮기기 전에 주장부터 댔다.**

    `[career]/tech-blog/assets/diagrams/` 에 picasso 그림 아홉이 1180px 로 있었고 저장소에는 없었다. **복사가 아니다** — README 열이 837, `docs/*.md` 가 1012 라 1180 을 그대로 넣으면 글자가 바닥(9.5px) 밑으로 내려간다. 그래서 폭을 인자로 받는 생성기로 다시 그렸고, **블로그용 1180 은 같은 파일에서 뽑는다.** 두 번 그리지 않기 위해서다.
