...
47. **관측선을 붙이기 전에는 축소가 열리지 않는다.** V8의 `robot_liveness`가 판정 앞에 서므로, 바인딩만 되고 한 번도 보고한 적 없는 기체가 하나라도 있으면 §9.3의 두 조회가 `NOT_OBSERVABLE`이다. **의도한 방향이다**(§15.41의 보수성) — 하지만 대가가 있다: 하트비트를 보내지 않는 어댑터를 붙이면 그 사이트의 축소 기능이 통째로 멈춘다. 기능이 죽는 것이지 사고가 나는 것은 아니다.
...
50. **`Preconditions`의 두 조회는 major를 안 본다.** `RegistryLedgerQuery`(CI 경로)는 `skill_type@major`까지 짚는데 운영 경로는 이름만 본다. 같은 §9.3을 구현하는 두 경로의 판정 범위가 다르며, 다른 major의 태스크가 축소를 막는다. 관측선은 공유 클래스로 합쳤지만 이 차이는 남았다.
...
52. **`derived_from`이 지금은 참조 프로파일의 합성 값이다.** `profile/profiles/`의 셋은 `vendor: picasso-ref`이며 실물 벤더 문서에서 파생하지 않았다(§7.4). 그래서 진단 7번의 대조는 **아직 무엇도 잡지 않는다** — 자리와 규율을 먼저 만든 것이고, 실물 기종이 오면 그 값이 진짜 파생 근거를 담아야 한다.
