...
148. **효과가 관측을 만나 «모른다» 를 줄였다 — 그리고 표의 한 줄은 겨냥할 대상이 없었다.**

    설계안 §5. 결함 통지는 «스킬이 실패했다» 까지만 말하고 적재가 어디 있는지는 말하지 않는다. 카탈로그의 효과가 있으면 그 자리를 채울 수 있다 — 놓고 끝나야 하는 `pick_place` 가 든 채로 끝났으면 **미완료 파지**다. `IN_DOUBT` 는 정직하지만 정보가 아니고, «아직 들고 있다» 는 다음 행동을 가른다.

    **관측이 없으면 판정하지 않는다.** `NOT_OBSERVABLE` 과 침묵은 둘 다 판정 대상이 아니다. 여기서 뚫리면 선언을 근거로 현실을 단정하는 것이 되고, 관측 경로가 죽었을 때 고장난 기체가 멀쩡해 보인다 — 이 저장소가 파지·진행률·결함에서 반복해 거절한 접기다. 미믹은 못 보는 기종이 아니라 그 상황을 못 만들어, 파지를 가리는 포트를 시험 쪽에 뒀다(엔진을 고치면 다른 시험들이 기대는 성질이 흔들린다).

    **표 첫째 줄(들고 끝나는 능력 + 빈손 → 적재 유실)은 카탈로그에 대상이 없다.** 효과를 선언한 스킬이 `pick_place` 하나뿐이고 쥐고 놓기를 둘 다 하므로, 쥐고 놓지 않는 스킬이 없다. 결함 주입이 그것을 드러냈다 — 그 가지의 방향을 뒤집어도 아무 시험도 빨개지지 않았다. 닿지 않는 가지는 규칙이 아니라 주석이다. 비교를 `compare(expected, observed)` 로 갈라 양방향을 직접 겨냥하고, 쥐고 놓지 않는 스킬이 카탈로그에 생기는 날 빨개지는 시험을 뒀다.

    적재 유실 통지가 온 실패는 어긋남이 아니다 — 미믹이 그때 빈손으로 두므로 효과와 관측이 맞는다. 이미 답한 자리에 두 번째 분류를 적지 않는다(§8 마지막 줄).
