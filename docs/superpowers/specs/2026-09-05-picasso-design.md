...
46. **폴백 파일을 다시 밀어 넣는 경로가 없다.** 핸드셰이크(`FileHandshakeReporter`)와 태스크(`FileTaskObservations`) 둘 다 **적기만 하고 읽지 않는다.** 레지스트리가 돌아온 날 그 줄들을 적재 표면으로 보내는 것은 사람이 해야 하며, 그때까지 원장은 그 구간을 못 본 채로 있다 — 축소를 막는 쪽이라 사고는 안 나지만 **그 구간의 소비자·태스크는 영영 관측되지 않는다.** 형식을 적재가 읽는 규약(protobuf JSON)으로 맞춰 둔 것이 그 경로를 나중에 만들 수 있게 하려는 것이고, 줄 단위(JSONL)인 것은 프로세스가 죽어도 마지막 한 줄만 잃게 하려는 것이다.
