...
**2026-09-08 — 이 목록에서 "상위 시스템 ACL의 실제 연동"을 뺐다.** 층이 경계 안으로 들어왔으므로 계약 스위트가 그 층도 봐야 한다. 다만 **지금은 시험할 방법 자체가 없다** — 아래쪽의 `mimic`에 해당하는 것이 위쪽에 없기 때문이며, 그 공백은 §15.70에 적는다. 목록에서 빼는 것과 시험이 생기는 것은 다른 일이고, 여기서 일어난 것은 앞의 것뿐이다.
...
70. **위쪽에는 `mimic`이 없다.**
    ...
    아래쪽은 프로파일을 읽는 에뮬레이터가 있어서 **실물 로봇 없이** 계약을 검증한다. 그것이 이 프로젝트의 두 주장 중 하나를 성립시키는 축이다. 위쪽에는 그 축이 없다 — §9.6이 상위 연계 층을 경계 안으로 들인 순간, `acl-{system}`을 검증하려면 **고객사 MES가 실제로 와야 하는 상태**가 되었다.

    이것은 §15.7과 같은 실패 모드다. 거기서는 실물이 없는 동안 계약이 에뮬레이터에 맞춰 굳었고, 여기서는 반대로 **검증할 상대가 아예 없어서 아무것도 굳지 않는다.** 둘 다 "실물이 도착해야 알 수 있다"로 미뤄지는데, 위쪽은 그 실물이 사이트마다 다르므로 미루는 비용이 더 크다.

    **무엇을 만들어야 하는지는 아직 정하지 않았다.** 프로파일이 기종을 기술하듯 상위 시스템을 기술하는 선언이 있어야 하는지, 아니면 몇 개의 고정 시나리오면 되는지가 먼저다. 전자면 C-1의 위쪽 판이 하나 더 생기는 것이고, 후자면 `harness`의 픽스처로 끝난다. **그 판단이 서기 전에는 §12.3에서 뺀 줄이 빈 약속이다.**

71. **범위는 옮겼는데 층은 아직 설계되지 않았다 — 그리고 그 중간 상태가 비목표보다 위험하다.**
...
73. **`GetKnownSiteNames`를 실물 어댑터 셋 중 아무도 구현하지 않았다.** 계약에 있고 `mimic`이 답하지만, `adapter-unitree-g1`·`adapter-boston-dynamics-spot`·`adapter-agility-digit`의 남쪽 포트에는 그 질의가 없다.

    **어려워서가 아니라 각자 다른 곳에 물어야 하기 때문이다** — Spot은 업로드된 GraphNav 그래프의 웨이포인트, Digit은 `ObjectSelector`로 고르는 세계 모델, G1은 물어볼 데가 없어 `unsupported`. 셋을 붙이는 것은 어댑터마다 한 메서드이고, **그때까지 실물에서는 이 확인이 안 선다.**
