...
진단은 **read-only JSON 엔드포인트 여덟**으로 낸다. 화면은 이를 그대로 표로 그리는 한 장이며 폴링(기본 10초)한다.
...
59. **계약 카탈로그에 아무 실물도 선언하지 않는 스킬이 둘 있다.** `pick_place`와 `inspect`다. 게이트 4번은 **선언된** 스킬만 양방향으로 대조하므로 쓰이지 않는 카탈로그 항목을 잡지 않고, 잡을 이유도 없다 — 어휘가 실물보다 앞서는 것 자체는 정상이다.
...
60. **시맨틱 결속 층에 주인이 없다**([ADR 34](../../adr/0034-semantic-binding-unowned.md)). 계약의 파라미터 넷이 사이트 수준의 이름인데(`location`·`object_id`·`destination`·`target`) 벤더는 녹화된 웨이포인트 id·픽셀·3D 점·카메라 이름으로 말한다. `navigate_to`만 맞는 것은 GraphNav이 그 결속을 이미 갖고 있어서다.
    ...
    ADR 34는 후보 셋 중 **어댑터를 뺐을 뿐**이다. 나머지 둘(사이트 데이터로 `registry`가, 또는 상위 시스템이)은 미정이며, 상위가 갖는 쪽이 가장 그럴듯한데 **벤더가 그 형태를 안 받으면 상위도 못 채운다**(픽셀·3D 점). 그래서 `pick_place`는 지금 어느 경로로도 서지 않는다.
...
65. **Digit 측정의 근거 등급이 다른 둘보다 낮다.** Spot은 벤더의 공개 proto 원문을 읽었고 G1은 공개 SDK 소스를 읽었는데, Digit은 그러지 못했다 — `docs.agilityrobotics.com`이 닿지 않고 공식 SDK가 공개 저장소에 없다. 대신 **원시 JSON을 그대로 쓰는 제3자 코드**(서브프로토콜 `json-v1-agility`)와 2026-09-05 조사(매뉴얼 직접 인용 포함)를 썼다.
...
66. **`move_relative`를 벤더 중립이라 부른 근거의 절반이 틀렸다.** 계약에 넣으며 *"속도와 지속시간은 Spot·Digit·AMR이 모두 갖는 이동 원시"*라 적었는데, Digit의 `action-move`는 **지속시간을 안 받는다**(같은 어휘의 `action-stand`에는 있으므로 개념이 없는 것이 아니라 이 액션에 없다). 스킬 선택 자체는 유지된다 — 셋 다 든다. 틀린 것은 근거이며 `skill_catalog.proto`의 주석을 고쳤다.

    **대가가 운영에 남는다.** 정지 시점을 Digit 어댑터가 소유하므로 **실패 방향이 다른 둘과 반대다** — G1(`SetVelocity`의 `duration`)·Spot(`SE2VelocityCommand`의 `end_time`)은 어댑터가 죽으면 로봇이 스스로 서는데, Digit은 **계속 걷는다.** 어댑터가 `stopDueAt`으로 그 빚을 드러내지만 고치지는 못한다 — 감시하는 쪽이 이 모듈 밖에 있어야 하고 지금은 없다.

67. **계층 열거의 가운데가 있다는 것을 표본 둘로는 못 봤다.** `profile/distance/`의 `vendor_layer`를 `NONE`·`COMMAND`·`MISSION` 셋으로 둘 때 가운데 값이 진짜로 쓰일지 확신이 없었다. Digit이 그 자리다 — 액션마다 상태 스트림이 오고 `action-sequential`·`action-concurrent`로 합성되는데 **일시정지·취소·재시작이 없다.**

    셋이 한 값씩 차지하는 것은 지금 우연이다. 넷째 실물이 이 열거로 안 접히면 값을 늘려야 하며, 그때 **늘리는 것이 옳지 이미 있는 값에 억지로 넣는 것이 아니다** — 이 열거의 쓸모는 기종을 분류하는 것이 아니라 *계약이 어느 층에 있는가*를 재는 것이기 때문이다.

68. **사이트 이름을 로봇에 등록했는지를 아직 아무도 안 본다**([ADR 35](../../adr/0035-site-names-live-in-the-robot.md)). 그 결정이 §9.7의 여섯 단계 옆에 새 운영 단계를 하나 만든다 — *"사이트 이름을 이 기체에 등록했는가."* 안 하고 바인딩하면 소비자가 `dock-3`을 보냈을 때 **로봇이 모르는 이름으로 실패**하고, 그 실패는 계약 위반처럼 보이지 원인처럼 보이지 않는다.

    **상태를 만들지 않았다.** ADR 35의 결정 3(`registry`가 그 사실을 §9.7 ④의 `conformance_status`와 같은 자리에 둔다)은 아직 스키마에도 진단에도 없다. 그래서 지금은 *"우리는 아직 안 했다"* 가 화면에 안 보이며, §15.47이 `liveness`로 닫은 것과 §9.7 ④가 `UNTESTED`로 닫은 것이 여기서는 **열려 있다.**

    닫는 비용은 작다 — `robot_binding`에 열 하나와 진단 1번에 필드 하나다. 안 닫아 둔 이유는 **등록의 형태가 기종마다 다른 것이 아직 확인되지 않아서**(§15.65의 Digit `AddObject`) 무엇을 확인했다고 적을지가 정해지지 않았기 때문이다. Digit 쪽이 확인되면 그때 만든다.
