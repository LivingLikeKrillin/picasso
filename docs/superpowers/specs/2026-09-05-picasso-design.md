...
30. **MQTT 발행이 실제 브로커로 검증되지 않았다.** 2단계는 발행자 추상만 두고 토픽·페이로드·순서 규약을 in-process로 증명한다. 이유는 §12.1의 결정성 규율이다 — 내장 브로커는 자기 스레드를 벽시계로 돌리므로 "시드 + 가상 시계 고정 = 동일 이벤트 시퀀스"가 깨진다. **증명되지 않는 것 넷:** `CONNECTION_BROKEN`이 브로커 Last Will이라는 것(§4.7), `retain`(§4.7·§9.6), §10.6의 단절 중 버퍼링과 넘칠 때의 새 `session_id`, 그리고 토픽 문자열의 브로커 수준 합법성(`+`·`#`·빈 레벨).
...
35. **진단 2번이 선택 필드 차이를 못 본다.** `profile_optional_field`가 아직 없는 표라 `optionalFields`가 `"not_tracked"`로 나간다. 빈 배열로 내지 않는 것이 요점이다 — 그러면 "선택 필드 차이가 없다"는 거짓말이 되고, 그 위에서 운영자가 축소를 승인한다.
...
38. **HTTP 표면에 인증이 없다.** 진단 넷은 read-only지만 바인딩·거절·요구 문자열이 그대로 나간다. 배포에서 망 경계로 막는 것을 전제하며, §8.5의 승인 경계는 **조작**에만 걸려 있다. 조작 API가 붙는 순간 이것은 한계가 아니라 결함이 된다.
...
40. **원장의 두 관측선이 채워지되 태스크 쪽은 브로커가 아니다.** 4a가 이것을 고쳤다 — `consumer_requirement`는 `mimic`의 `HandshakeReporter`가 §5.4대로 보고해 채우고(HTTP, `mimic ⇢ registry`), `task`는 `IngestBridge`가 발행을 받아 넘긴다. **후자는 §3.2가 규정한 `registry ⇠ 브로커` 구독이 아니다** — 브로커를 붙이면 위 30번과 같은 이유로 §12.1의 결정성이 깨지므로, 그 자리를 in-process 어댑터가 대신한다. 넘기는 것이 `StateMessage`(계약 타입)라 브로커가 붙는 날 어댑터만 갈아 끼우면 되고, `registry`는 자기가 어댑터에게서 받았는지 브로커에게서 받았는지 몰라도 된다. 남는 것: **운영 배포에서 그 어댑터를 무엇이 들고 있는가가 아직 없다** — `harness`가 시험에서 들고 있을 뿐이다.
41. **관측선 신선도 창이 24시간 상수다.** 사이트별·스트림별 설정이 없고 코드에서만 주입된다. 짧아서 생기는 오답은 "모른다"라 축소를 **막고**, 길어서 생기는 오답은 죽은 구독을 살아 있다고 보아 축소를 **연다** — 그래서 애매하면 짧은 쪽으로 잡았다. 야간 정지가 24시간을 넘는 라인에서는 매번 막히며, 그때 필요한 것은 창을 늘리는 것이 아니라 관측선을 붙이는 것이다(위 40번).
42. **사이트 카탈로그는 집계까지다.** §9.6의 `picasso/{major}/{site}/site/catalog`는 retain 스트림인데 브로커가 없고(위 30번), HTTP 표면도 안 만들었다. `SiteCatalog`가 **무엇을 답하는가**까지 정하고 내보내는 쪽은 비어 있다 — 발행자를 지금 만들면 아무 시험도 안 지나는 코드가 생긴다.
43. **`skill_type_deprecation`에 쓰기 경로가 없다.** 표와 읽는 쪽(전제 조건 `DEPRECATION_PUBLISHED`, 사이트 카탈로그의 계약 축)은 있지만 계약 축 예고를 등록하는 API는 없다. `skill_type`이 읽기 전용인 것과 같은 이유로(§8.1) 계약 소유자가 채우는 것이 옳고, 그 동기화 잡이 `SkillTypeSync`에 아직 없다.
44. **드레인 관측의 해상도가 발행 주기에 묶여 있다.** `task` 적재는 `state` 스트림의 주기 발행(§7.2의 `publish_interval`)을 받는데, **주기보다 짧은 태스크는 비종착으로 한 번도 안 실릴 수 있다** — 픽스처의 `navigate_to`가 20초이고 발행 상한이 30초라 실제로 그렇다. 그러면 §9.3의 드레인이 그 태스크를 못 보고, 도는 중인데도 0으로 관측된다. 틀리는 방향이 **축소를 여는 쪽**이라 위 41번의 보수성과 반대다. 전이마다 발행하거나(§4.5의 `event` 스트림을 함께 적재) 주기를 짧게 하면 닫히며, 전자가 옳다 — `event`에는 전이가 다 있고 놓칠 수 없다.
45. **적재 실패가 조용하다.** §5.4가 *"보고 실패는 핸드셰이크 결과에 영향을 주지 않는다"*고 정했으므로 `SkillServiceImpl`이 삼키고, `IngestBridge`도 같은 이유로 삼킨다. 잃지 않는 것은 `FallbackHandshakeReporter`(파일)의 몫이고 **태스크 쪽에는 그 폴백이 없다.** 그래도 조용히 위험해지지는 않는다 — 적재가 멈추면 워터마크가 늙어 §9.3의 조회가 `NotObservable`이 되고, 그것은 축소를 **막는** 쪽이다(§15.41). 다만 **왜 막히는지**는 로그가 없어 알기 어렵다.
