...
196. **옆 시스템과 주고받을 것을 적었다 — 소유자를 못 정해도 계약까지 비워 둘 이유는 없었다.**

    `orchestration.md` 는 L1 과 L3 의 자리와 방향을 확정해 두었지만 **무엇을 어떤 순서로 주고받는지**는 비어 있었다. 비어 있는 경계는 «범위 밖» 이 아니라 미정의 경계이고, 연동하는 날 배치마다 다르게 채워진다. [옆 시스템과의 통신](2026-09-25-peer-systems-dispatch-and-space-design.md)이 그것을 적는다. 짓지는 않는다 — 소비자가 없다(ADR 9).
