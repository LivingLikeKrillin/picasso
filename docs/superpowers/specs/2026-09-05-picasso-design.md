...
30. **MQTT 발행에 실제 구현이 생겼다. 기본 스위트는 여전히 in-process다.** `MqttPublisher`(Paho v5)가 발행자 추상을 채우고, `MqttBrokerTest`가 컨테이너 브로커(Mosquitto)로 §15.30이 꼽았던 넷 중 **셋을 닫는다** — `retain`(붙는 순서를 바꿔 양쪽으로 관측), Last Will이 `CONNECTION_BROKEN`이라는 것(강제 절단 vs 정상 종료로 양쪽), 토픽 문자열의 브로커 수준 합법성(`Topics`가 막는 집합과 브로커가 거부하는 집합이 같은지).

    **기본 스위트는 안 바뀐다.** §12.1의 결정성 — "시드 + 가상 시계 고정 = 동일 이벤트 시퀀스" — 이 브로커를 붙이면 깨지므로 `mimic`의 시험은 하나도 컨테이너를 안 쓴다. 브로커 시험을 `harness`에 둔 것도 그래서다(거기는 이미 Docker를 요구한다, 아래 39번).

    **여전히 증명되지 않는 것:** §10.6의 단절 중 버퍼링과 넘칠 때의 새 `session_id`. 그것은 브로커가 아니라 `mimic`의 거동이고, 브로커를 끊었다 붙이는 시나리오가 필요하다. 그리고 **발행 경로는 아직 이 구현을 안 쓴다** — 운영 결선(위 40번)이 in-process 발행자에 적재를 감싸는 형태라, `MqttPublisher`를 실제로 무는 것은 브로커를 배포하기로 정하는 날이다.
