...
68. **사이트 이름을 로봇에 등록했는지를 아직 아무도 안 본다**([ADR 35](../../adr/0035-site-names-live-in-the-robot.md)). 그 결정이 §9.7의 여섯 단계 옆에 새 운영 단계를 하나 만든다 — *"사이트 이름을 이 기체에 등록했는가."* 안 하고 바인딩하면 소비자가 `dock-3`을 보냈을 때 **로봇이 모르는 이름으로 실패**하고, 그 실패는 계약 위반처럼 보이지 원인처럼 보이지 않는다.
    ...
    **상태를 만들지 않았다.** ADR 35의 결정 3(`registry`가 그 사실을 §9.7 ④의 `conformance_status`와 같은 자리에 둔다)은 아직 스키마에도 진단에도 없다. 그래서 지금은 *"우리는 아직 안 했다"* 가 화면에 안 보이며, §15.47이 `liveness`로 닫은 것과 §9.7 ④가 `UNTESTED`로 닫은 것이 여기서는 **열려 있다.**
    ...
    닫는 비용은 작다 — `robot_binding`에 열 하나와 진단 1번에 필드 하나다. 처음에는 *"등록의 형태가 기종마다 다른 것이 아직 확인되지 않아서"* 미뤘는데 **같은 날 확인됐다**: Spot은 지도 녹화 시의 웨이포인트 명명, Digit은 `add-object`·`add-landmarks`·`set-floorplan-map`, G1은 자리 자체가 없다.

    그래서 미룰 이유가 바뀌었다. 이제 걸리는 것은 **무엇을 등록했다고 적을 것인가**이며 그 집합이 기종마다 다르다 — Spot은 장소뿐이고 Digit은 장소와 집을 물체와 놓을 곳까지다. 열 하나에 불리언을 넣으면 *"등록했다"*가 기종마다 다른 것을 뜻하게 된다. 그 형태를 정하는 것이 이 항목을 닫는 일이며 아직 안 정했다.
