...
조작 열다섯: 기종 등록 / 기체 등록 / **어댑터 등록** / **어댑터 버전 등록** / 개정판 올리기(DRAFT 생성·편집·재제출) / 시험 요청 적재 / 바인딩(카나리 포함) / 롤백 / 개정판 폐기 / 능력 차단·해제 / **계약 축 폐기 예고 설정**(`skill_type_deprecation`) / **소비자 요구 등록**(`POST /requirements`) / **변경 계획 생성** / **변경 계획 단계 실행** / **사이트 이름 등록 기록**(`POST /operations/site-names`, ADR 35).
...
79. **등록의 *존재*는 확인하는데 등록의 *충분함*은 확인하지 못한다.**

    `GetKnownSiteNames`가 답하는 질문은 하나다 — *"이 이름을 기체가 아는가."* CLAIMED / CONFIRMED / CONTRADICTED가 그 위에 선다(ADR 35). **그런데 일감은 이름 이상을 요구한다.** *"1번 창고 6번 자리에 놓아라"*는 그 자리의 **높이·접근 방향·자세**를 요구하고, 그것은 이름이 무는 결속이 충분한가의 문제다.

    **Spot 원문으로 재면 이름과 자세가 다른 것을 문다.** 이름은 `Waypoint.Annotations.name`에 붙고 벤더 예시가 *"Kitchen Fridge"*다. 웨이포인트가 무는 자세는 `Waypoint.waypoint_tform_ko`(`SE3Pose`)인데 그것은 **로봇이 서서 측위하는 자리**다. `Annotations`의 나머지 스물몇 필드는 전부 항법·측위·GPS·루프 클로저·충돌 설정이고 **조작 대상의 자세는 없다.**

    ⇒ **충분함은 스킬마다 다르다.** 같은 이름이 `navigate_to`에는 충분하고(웨이포인트가 정확히 그것을 위해 있다) `pick_place`의 `destination`에는 모자랄 수 있다. **CONFIRMED가 나와도 그 이름으로 놓기가 되는지는 모른다.**

    그 결속을 무는 자리는 딴 데 있다 — 대상 공간의 `WorldObject.transforms_snapshot`(전체 프레임을 나르지만 §15.78대로 **만료된다**), 또는 Autowalk `Element`의 `action`·`action_wrapper`(저작 시점에 붙는 액션 파라미터). 둘 다 우리가 지금 안 보는 곳이다.

    **여기서 무엇을 할지 정하지 않는다.** 높이를 계약에 값으로 실으면 §15.78의 주소 공간이 위로 새고, ADR 34가 어댑터에게 맡긴 번역을 상류가 하게 된다. 그렇다고 등록이 충분한지를 아무도 안 보면 **운영자는 초록을 보고 배치하는데 놓기만 실패한다** — §9.7 ④가 *"안 했다가 화면에 보여야 정직하다"*고 한 자리와 같은 종류이며, 지금은 그 화면조차 없다.

    > **곁가지 하나.** `Waypoint.Annotations.waypoint_source`가 `ROBOT_PATH`(녹화 중 로봇 경로에서 자동 생성) · `USER_REQUEST`(사람이 놓음) · `ALTERNATE_ROUTE_FINDING`을 가른다. **벤더가 저작 여부를 선언해 준다는 뜻이다.** 지금 Spot 어댑터의 `knownSiteNames()`는 *"이름이 비지 않았는가"*로 저작물을 짐작하는데(§15.73), 이 필드가 그 짐작을 관측으로 바꿀 수 있다. 아직 안 든다.
