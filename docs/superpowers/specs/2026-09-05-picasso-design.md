...
194. **대장의 갈래가 여섯 달 동안 조용히 어긋나 있었다 — 갈래의 뜻이 「누가 닫는가」인데 그것을 아무도 안 댔다.**

    한계 대장은 갈래를 셋으로 두고 있었다. 의도적 제외, 내부 미결(이 저장소가 지으면 닫힌다), 외부 미결(실물이 와야 닫힌다). 그런데 내부 미결 마흔 줄 중 **열셋의 해소 조건이 밖의 주어를 대고 있었다** — 「받는 쪽이 지표에서 제외할 때」, 「읽는 쪽이 그 칸으로 무엇을 말할지 정해질 때」, 「소비자가 생기기 전에는 짓지 않음」. 지으면 닫힌다고 적은 칸에 **지어도 안 닫히는 것**이 앉아 있었다.
...
193. **내보내기 절차를 한 벌로 합쳤다 — 막고 있던 「요약이 흔들린다」는 재 보니 사실이 아니었다.**
    ...
    ★★**그 위험이 없었다.** 요약은 `IncidentBundle.digest()` 가 내고 줄은 `LedgerExport` 가 적는데, 갈려 있던 것은 **그 뒤에 오는 담는 절차**다 — 디렉터리를 만들고, 임시 이름으로 쓰고, 이름을 바꾸고, 안내를 마지막에 둔다. 인코더를 한 글자도 안 건드리므로 요약이 흔들릴 경로가 애초에 없다. 합치기 전에 네 벌의 요약을 받아 두고 합친 뒤에 다시 받아 댔다 — 9·9·4·3 이 전부 같고, 커밋된 인계본의 것과도 같다. **엿새를 막은 것은 대가가 아니라 안 재 본 전제였다**(§15.191 과 같은 병이 사흘 사이 두 번째다).
