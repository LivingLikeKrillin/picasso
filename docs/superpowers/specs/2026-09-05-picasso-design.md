...
| 9 | 즉시 명령과 동시성 어휘를 계약에서 뺀 것 — 소비 표면이 없는 선언은 두지 않는다 | — |
| 10 | 투영에 들어가는 것과 발신자 내부 설정을 나눈 기준 | — |
...
| 22 | `gate`를 라이브러리로 두고 CI·registry가 기준선만 달리해 공유, 6번은 언제나 문서를 diff | — |
| 23 | 게이트 7번을 문자열 검사로 둔 것 | → §15.6 |
