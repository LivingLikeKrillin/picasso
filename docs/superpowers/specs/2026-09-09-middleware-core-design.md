...
규칙: 능력은 최고 등급을 선언하고, 요청은 요구 등급을 지정하고, 미달이면 `UNVERIFIED`(도달 등급 표기). *"확인 수단이 없으면 제공 불가"* 는 E2 이상을 요구하는 업무에만. **`[결정 필요]`** — 도달 등급을 계약(④)의 종착 갱신에도 실을지(어댑터가 E0/E1 을 안다), 실행 층에서만 들지. 보고서는 능력 계약에 두므로 기본은 실행 층이고, ④에는 어댑터의 문 종류만 오르면 된다.
...
`[결정 필요]` — 분류표. 아래는 측정된 벤더 어휘(Spot 조작 18·항법 14·미션 8, Digit 상태 4, G1 에러 11·종료 7)에서 **여럿이 실제로 내는 뜻**을 뽑은 후보다. 검토 뒤 확정한다.
...
새 모듈 **`middleware`** `[결정 필요: 이름]`. 의존: `contracts`, `client`(계약 소비자), `profile-model`. **`registry` 와 분리** — registry 는 개정판·원장·변경 계획이고 이것은 실행이다. 게이트 7번(기종 분기 금지)의 대상에 **넣는다**.
...
1. **정준 모델 스키마 확정** — 이 문서 §1 의 `[결정 필요]` 셋(도달 등급의 위치 · 실패 분류표 · 모듈 이름) 검토 → ADR 38 확정.
