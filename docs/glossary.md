...
> **왜 이 칸이 생겼나.** 재작업은 근거 창의 `@r1` 로도 드러나지만 **재작업 뒤에 그 단위가 다시 사건을 열었을 때에 한합니다**(`run-3` 의 둘째 사건). 재작업 뒤에 아무 일도 안 난 단위는 그 사실을 싣지 않았고(`run-4`), 그래서 사건과 그 뒤의 탐색 사이가 비어 보여 읽는 쪽이 그 사이를 「자동으로 회복했다」로 메울 수 있었습니다. **데이터 옆에 산문으로 들고 다녀야 하는 사실은 데이터에서 빠진 사실입니다**(§15.188).

> 마지막 대조: 2026-09-23 · sha256:a58c8cebc9ed · 열림: §15.185
