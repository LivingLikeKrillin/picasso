...
| 자원 | 소유자 | 관문 (승인 지점) | 현재 |
|---|---|---|---|
| 셀 내 기체의 작업 배정 | 본 저장소 | `Middleware.submit` → `chainViolation` · `liveHold` | 있음 |
| 셀 내 슬롯·지그·버퍼 점유 | 본 저장소 | — | **없음** (§15.153) |
| 셀 내 작업 공간(반경 겹침) | 본 저장소 | — | **없음** (§15.153) |
...
| 자리 이름 ↔ 좌표 결속 | 기체 세계 모델 또는 상위 사이트 정본 (ADR 34 §4) | 기체 질의 (ADR 35) 앞에 `SiteBindingCheck` — 활성 지도 판과 다르면 풀지 않음 | 부분 (§15.155 — 개체 캘리브레이션 판은 아직) |
...
| **범위 안이고 미구현** | 셀 내 자원 점유 축(§15.153), 자리 결속의 기준 버전 무효화(§15.155), 배정 제안 포트, 비용 함수, 진동 방지 |
...
> 마지막 대조: 2026-09-17 · sha256:8d6130ceecee · 열림: §15.143, §15.153, §15.154, §15.155
