...
| 셀 내 슬롯·지그·버퍼 점유 | 본 저장소 | `Middleware.submit` → `occupancyViolation` — 자리 경쟁과 출발 결품. 출발 결품은 그 자재를 든 다른 자리를 계산해 대장에 남김 | 있음 (§15.183 — 자리 경쟁은 대장에 안 남는다) |
...
**요청하는 쪽의 계약은 따로 적었습니다.** 위 표는 본 저장소가 **구간 소유자일 때** 내놓을 관문입니다. 본 저장소가 걷는 기체를 대신해 **다른 소유자에게 요청하는** 쪽의 메시지·순서·실패 처리와, 배정 정책(L1)과 주고받는 것은 [피어 시스템과의 통신](superpowers/specs/2026-09-25-peer-systems-dispatch-and-space-design.md)이 정합니다. 소유자가 누구로 정해지든 그 계약은 같습니다.
...
**파일로 쓰는 쪽은 시나리오 구동기입니다.** v1 에서 그것은 시험이며, 한 벌은 `picasso/build/export/run-1` · `run-2` · `run-3` 에 나옵니다. **앞의 둘은 같은 시나리오를 두 번 돌린 것입니다** — 두 벌의 해시가 같고 `runId` 만 다른 것이 그 값이 존재하는 이유이고, 읽는 쪽은 그 두 벌로 «둘째 구동이 새 사건으로 들어온다» 를 회귀로 박습니다. **`run-3` 은 재실행이 아니라 다른 시나리오입니다** — 같은 기체가 같은 분류로 거듭 깨지는 한 벌이며, 재발을 세는 쪽이 그 셈을 실물로 한 번 밟게 하는 자리입니다(§15.182). 따라서 **시험을 돌려야 한 벌이 갱신됩니다** — C 를 고른 대가이고, 승격하면 그 제약이 사라집니다.
...
| 대장의 갈래 | 한 줄에 **평평하게** 적습니다. `outcome` 이 `FOUND` · `NONE` · `WITHHELD` · `SOURCE_MISSING` 중 하나입니다. 앞의 셋은 능력 사슬의 탐색이 낸 답이고 `SOURCE_MISSING` 은 셀 자리의 점유가 낸 답이며, **한 대장에 두는 이유는 접수 관문이 첫 거절에서 되돌아가기 때문입니다** — 한 번의 접수는 많아야 한 번 거절되고 그 거절에 대한 답은 하나뿐입니다(§15.183) |
| 출발 결품 | `SOURCE_MISSING` 이 `material` · `source` · `observed` · `alternatives` 를 싣습니다. **`alternatives` 는 널과 빈 목록이 다릅니다** — 널은 셀이 그 질문에 답하지 않은 것이고 빈 목록은 그 자재를 든 자리가 하나도 없다는 답이며, 접으면 못 물어본 것이 재고 부족으로 읽힙니다. **`steps` 키가 없습니다** — 가려서가 아니라 승인할 것이 없어서이며, 어느 자리를 쓸지는 주문을 고치는 쪽의 결정입니다 |
...
| 의도 | `intent` 가 **상류가 적은 것과 이 층이 편 것을 함께** 싣습니다. 앞은 작업 마스터·판·주문 값·자재·설비이고 뒤는 스킬·단위 값·출발·도착·기대 식별자이며, 둘이 갈리는 자리가 곧 이 층의 번역입니다. 능력의 최고 등급과 시간창 폭도 함께 실어 `evidenceWindow` 가 **왜 그 범위였는지**를 답합니다 |
...
| 판 | `schemaVersion` 이 `5` 입니다. `2` 에서 `blockedBy` 가 분류 이름의 배열에서 결함 객체의 배열로 **모양이 바뀌었으므로** 그 판은 읽는 쪽이 보아야 하며, `3` 은 칸이 는 것뿐입니다. `4` 는 사건 줄을 바꾸지 않고 **탐색 줄의 `outcome` 에 갈래를 하나 더합니다** — 칸이 느는 것과 달리 `outcome` 으로 분기하는 읽는 쪽은 모르는 값을 만나므로 그 자리만 판을 보면 됩니다 · `5` 는 사건 줄에 `resolution`(사람이 그 단위에 낸 판단) 한 칸을 더합니다 — 칸이 느는 것이라 읽는 쪽이 안 보아도 깨지지 않고, **요약은 안 바뀝니다**(해시에서 빠지는 셋째 칸입니다) |
...
| `NOT_DECLARED` | **원래 없었습니다.** 선언을 등록합니다 |
| `REVOKED` | **전에 허락되었다가 회수되었습니다.** 철회 사유를 확인하고 **사후 검토로** 갑니다. 선언을 다시 올리는 자리가 아닙니다(ADR 45) |
| `EXPIRED` | 기간이 지났습니다. 갱신합니다. **철회가 이보다 먼저 판정됩니다** — 둘 다인 경우 만료를 먼저 내면 기간만 연장되어 철회가 조용히 되돌아갑니다 |
...
**철회는 사람의 승인을 막지 않습니다.** 사람은 선언 없이 승인하므로 철회가 라인을 정지시키지 않습니다. 정지시킬 경우 아무도 철회를 사용하지 않게 되며 그 시점에 본 장치가 무력화됩니다 — 만료를 짧게 설정할 수 있는 근거와 동일합니다.

**철회의 기록은 선언에 표시로 남으며 본 계층은 그것을 읽기만 합니다**(ADR 45). 삭제로 구현할 경우 선언 조회가 널을 반환하고, 그것은 `NOT_DECLARED` 와 동일한 형태가 되어 후속 조치가 정반대인 두 상태가 접힙니다. 자격 강등의 주체는 선언의 소유자이며, 자동 승인한 조치의 실패 사실은 사건 번들의 `approvedBy` 로 이미 외부에 전달됩니다(§15.168).
...
**해시에 드는 것과 아닌 것.** 봉인 시점에 확정되고 같은 시드면 같은 값인 것은 전부 듭니다. 새로 들어온 기체·근거 등급·대조 결과·걸음 위치·결함 원문·의도·경로·관측 신뢰가 모두 그러하며, 그중 하나가 다른 사건은 다른 사건입니다. 실 시계(`wallClockAt`)와 나중의 판정(`review`)만 빠집니다. **맵은 키 순서로 정렬해 적습니다** — 순회 순서가 해시를 바꾸면 같은 사건이 두 값을 갖습니다.

> 마지막 대조: 2026-09-25 · sha256:d3ac15a5a659 · 열림: §15.143, §15.154, §15.162, §15.163, §15.3, §15.166, §15.173, §15.174, §15.175, §15.176, §15.178, §15.183, §15.184
