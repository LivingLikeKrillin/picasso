...
| 판 | `schemaVersion` 이 `4` 입니다. `2` 에서 `blockedBy` 가 분류 이름의 배열에서 결함 객체의 배열로 **모양이 바뀌었으므로** 그 판은 읽는 쪽이 보아야 하며, `3` 은 칸이 는 것뿐입니다. `4` 는 사건 줄을 바꾸지 않고 **탐색 줄의 `outcome` 에 갈래를 하나 더합니다** — 칸이 느는 것과 달리 `outcome` 으로 분기하는 읽는 쪽은 모르는 값을 만나므로 그 자리만 판을 보면 됩니다 |
...
> 마지막 대조: 2026-09-22 · sha256:feb7bff81505 · 열림: §15.143, §15.154, §15.162, §15.163, §15.3, §15.166, §15.173, §15.174, §15.175, §15.176, §15.178, §15.183, §15.184
