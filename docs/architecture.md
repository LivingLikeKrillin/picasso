...
<picture>
  ...
  <img alt="띠 셋. 위는 상위 시스템(MES·WMS·SCADA·ERP·라인 제어기)이고 무엇이 와도 된다, 가운데는 계약과 레지스트리이고 여기만 고정이다, 아래는 기체와 설비(G1·Spot·Digit·Orbit 플릿·산업 설비·미믹)이고 역시 무엇이 와도 된다. 띠 사이의 경계 둘에 각각 소비자와 어댑터가 적혀 있다." src="diagrams/three-layers.svg">
</picture>

**가운데만 우리가 정한다.** 위아래는 구현하지 않은 것이 아니라 무엇이 와도 상관없게 만든 것이고, 그 상관없음을
지키는 것이 게이트 검사 7번이다.

★**아래 §1 의 「층」과 다른 축이다.** 여기 셋은 **누가 무엇을 구현하는가**(배치의 경계)이고, §1 의 넷은
**어휘가 어디서 갈리는가**(ADR 36)다. 같은 낱말을 쓰지만 세는 대상이 다르다.
...
<picture>
  ...
  <img alt="위에서 아래로 세 칸: 기종이 무엇을 드는지 선언하는 프로파일 JSON, 요구와 선언을 맞춰 보는 Negotiator, 그리고 어댑터의 accept. 그 아래에 계약 스킬 셋이 왼쪽에, Spot 의 벤더 표면 다섯이 오른쪽에 있고 선으로 이어진다. move_relative 는 명령 계층 하나, navigate_to 는 그래프와 미션 둘, inspect 는 세계 모델과 취득 둘을 쓴다. 묻기만 하는 표면으로 가는 선은 파란 점선이다. 표면마다 그것이 없으면 어느 스킬이 죽는지가 적혀 있다." src="diagrams/skill-mapping.svg">
</picture>

**매핑은 스킬 단위가 아니라 표면 단위다.** `navigate_to` 하나가 표면 둘을 쓰고 그 둘의 역할이 다르다 —
그래프 계층은 사이트 이름을 웨이포인트 id 로 **묻기만** 하고(ADR 34·35), 미션 계층이 **시킨다**. 그래서 표면
하나가 빠지면 그것을 쓰는 스킬만 `VENDOR_SURFACE_ABSENT` 로 거절되고 **나머지는 그대로 돈다** — 명령 계층만
있는 Spot 은 `navigate_to` 를 못 들지만 `move_relative` 는 받는다.

**기종 사이의 거리는 스킬마다 다른 것이 아니라 계층이 어긋난 것이다.** G1 에는 미션 계층에 해당하는 것이
아예 없고 `sport` 서비스가 명령만 준다. 그 사실이 프로파일의 `skills` 목록으로 먼저 드러나고, `Negotiator`
가 요구와 대조해 **안 맞는 것을 한 번에 전부** 돌려준다.
...
<picture>
  ...
  <img alt="왼쪽 지대에 비종착 여섯, 오른쪽 초록 지대에 종착 넷이 있고 지대를 건너는 선 넷은 전부 왼쪽에서 오른쪽으로만 간다. 오른쪽에서 왼쪽으로 가는 선이 하나도 없다." src="diagrams/task-states.svg">
</picture>

**종착에 든 태스크는 어떤 이유로도 비종착으로 돌아오지 않는다.** 그림에 오른쪽에서 왼쪽으로 가는 선이 없는 것이
그 규칙이다. `CANCELLED_RECOVERY_FAILED` 가 종착인 것이 요점이다 — 되돌리기에 실패해도 태스크는 끝난 것이고,
남은 것은 사람의 일이다.
