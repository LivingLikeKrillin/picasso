...
한계 대장(`limits.md`)에 등록된 미결 항목은 **60개**(내부 28개 · 소비자 대기 8개 · 외부 24개, 의도적 제외 13개 제외)이며, 그 상세 목록과 해결 조건은 `limits.md`에 명시되어 있습니다. 특히 실물 어댑터가 넷 있다(기체 셋, 플릿 하나). 다만, 어댑터 넷 중 어느 것도 실물에 붙여 보지 못했다(C-3)는 물리적 검증 한계가 존재하며, 이는 SDK 라이선스, JVM 바인딩 부재, 플릿 실기체 인스턴스 부재 등에 기인합니다.
...
| **미결 과제 대장** | [`docs/limits.md`](docs/limits.md) | 미결 한계 항목 60개(내부·소비자 대기·외부) 및 해결 조건 관리 대장 |
...
> 마지막 대조: 2026-09-23 · sha256:add881bca5b8 · 열림: C-3, §15.81
