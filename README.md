...
**계약.** 명령과 질의는 gRPC, 발행(상태·이벤트·연결)은 MQTT 다. 태스크는 `(task_id, revision)` 으로 멱등하고, 생명주기에 취소·일시정지·재시도·복구 실패·래치 위반·제어권 상실이 들어 있다 — 실물 조사에서 나온 것들이다. 계약은 **값이나 제약을 담지 않는다.** 그것은 프로파일의 몫이다.
