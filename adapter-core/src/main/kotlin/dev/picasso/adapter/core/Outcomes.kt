...
enum class Refusal {
    ...
    /**
     * 계약이 실은 사이트 이름을 **기체가 모른다**(ADR 35).
     *
     * 오타일 수도 있고 등록을 안 한 것일 수도 있으며, **어댑터는 둘을
     * 구별할 수 없다** — 구별하려면 올바른 이름의 목록을 어댑터가 가져야
     * 하고, 그 순간 시맨틱 결속의 주인이 바뀐다(ADR 34). 그래서 거절만 한다.
     */
    SITE_NAME_UNKNOWN,
     ...
     * 그 이름을 든 것이 둘 이상이다.
     *
     * **사이트 이름은 유일하지 않다.** Spot의 `Waypoint.annotations.name`은
     * 사람이 적는 문자열이고 둘을 막는 것이 없으며, Digit은 매뉴얼이
     * *동명의 객체 중 하나를 골라 쓰라고* `priority-order`를 둔다.
     * **임의로 하나 고르는 것이 가장 나쁘다** — 로봇이 다른 자리로 가고
     * 로그에는 성공이 남는다. 거절하면 사이트가 이름을 고친다.
     */
    SITE_NAME_AMBIGUOUS,

    /**
     * 그 이름은 **옛 지도 판에서 배운 것**이다(§15.155).
     *
     * 기체는 이름을 알고 풀 수도 있다 — 그래서 [SITE_NAME_UNKNOWN] 과 다르다.
     * 문제는 그 답이 지금 판의 답이 아니라는 것이고, 지도가 갱신된 뒤에는
     * **같은 이름이 다른 자리를 가리킨다.** 재등록 전까지 보내지 않는다.
     */
    SITE_BINDING_STALE_MAP,

    /**
     * 그 이름은 **옛 캘리브레이션 판에서 티칭한 것**이다.
     *
     * 지도는 그대로다. 기체 개체가 바뀌었고, 세계 모델을 복원해도 티칭의
     * 기준이 달라 같은 이름이 다른 자세를 뜻한다. **지도 축과 가르는 이유는
     * 사람이 할 일이 다르기 때문**이다 — 앞은 지도를 다시 찍는 일이고
     * 이것은 개체를 다시 티칭하는 일이다.
     */
    SITE_BINDING_STALE_CALIBRATION,

    /**
     * 결속 정본이 **그 이름을 모른다.**
     *
     * 어느 판에서 배운 것인지 알 수 없으므로 판정할 근거가 없다. 기체가
     * 우연히 풀 수도 있지만 그것은 이 층이 확인한 사실이 아니다 —
     * **확인 못 한 것을 통과로 접지 않는다.**
     */
    SITE_BINDING_ABSENT,
     ...
     * 결속 정본에 **못 물어봤다.**
     *
     * 앞의 둘과 달리 요청이 틀린 것이 아니다 — 우리 쪽 상류가 안 닿는다.
     * 그래서 계약의 거절이 아니라 gRPC 상태로 나간다([LINK_ERROR] 와 같은 자리).
     * 빈 답을 «판이 같다» 로 접으면 지도가 바뀐 뒤에도 명령이 계속 나간다.
     */
    SITE_BINDING_UNVERIFIABLE,
}
