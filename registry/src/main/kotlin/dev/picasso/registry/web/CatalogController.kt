 ...
 * ## 상위가 무엇인지 몰라도 된다
 *
 * 이 주석은 원래 *"§3.2가 상위 시스템 어댑터(ACL) 구현을 비목표로 두면서
 * 붙을 자리는 만든다고 했다"* 로 시작했다. **2026-09-08에 그 배치가 바뀌었다**
 * — 상위 연계 층은 경계 안이고(§9.6), 이 GET을 폴링하는 것은 경계 밖의
 * 누군가가 아니라 `acl-{system}`이다.
 *
 * **바뀌지 않은 것이 이 컨트롤러의 성격이다.** `registry`는 여전히 상대가
 * 무엇인지 알 필요가 없다 — MES-A용 어휘를 아는 것은 `acl-{system}`이고
 * 여기는 능력 한 줄을 그대로 준다. 소비자가 경계 안으로 들어왔다고 해서
 * 이 표면이 상위 시스템을 알기 시작하면, 아래쪽에서 게이트 7번이 막는
 * 것과 같은 것을 위쪽에서 하게 된다.
 *
 * ## retain 스트림 대신 폴링이다
 *
